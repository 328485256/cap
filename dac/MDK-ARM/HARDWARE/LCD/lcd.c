...
//画一个实心的点
//POINT_COLOR:此点的颜色
void LCD_Draw_Solid_Point(u16 x,u16 y,u16 color)
{
	POINT_COLOR = color;
  LCD_Draw_Circle(x,y,1);
	LCD_Fast_DrawPoint(x,y,color);
}
//输出采样波形，date默认为最大值为4096

void LCD_Draw_Output_waveform(u16 x_max,u16 y_max,u16 x_min,u16 y_min,u16 color,uint16_t data,uint16_t* Waveform_Date)//Length<400,370,760
{
//	static int Waveform_Date[400];
//	static int Waveform_Date[400];
	int Date_Disposed=0;
	int x,y,z;
	POINT_COLOR = BLACK;
	LCD_DrawLine(x_min,y_max,x_max,y_max);//横坐标线
	LCD_DrawLine(x_min,y_max,x_min,y_min);//横坐标线
	Date_Disposed = (int)(data/(4096/(y_max-y_min)+1));//将输入数据转化为y的坐标值，y_max-Date_Disposed才是最终输出的坐标
	//LCD_Draw_Solid_Point(x_min,y_max-Waveform_Date[0],WHITE);//清空原来的点（大点）
	LCD_Fast_DrawPoint(x_min,y_max-Waveform_Date[0],WHITE);//清空原来的点（小点）
	x=Waveform_Date[0];//保留原来点的数据
	Waveform_Date[0]=Date_Disposed;
	//LCD_Draw_Solid_Point(x_min,y_max-Waveform_Date[0],BLUE);//画出新的点（大点）
	LCD_Fast_DrawPoint(x_min,y_max-Waveform_Date[0],BLUE);//画出新的点（小点）
	//z=Waveform_Date[0];
	for(int i=1;i<(x_max-x_min);i++)
	{
		//delay_ms(100);
		POINT_COLOR = WHITE;
		LCD_DrawLine(x_min+i,y_max-Waveform_Date[i],x_min+i-1,y_max-x);//清空原来的线
	//	LCD_Draw_Solid_Point(x_min+i,y_max-Waveform_Date[i],WHITE);（大点）
		LCD_Fast_DrawPoint(x_min+i,y_max-Waveform_Date[i],WHITE);//清空原来的点（小点）
		y=Waveform_Date[i];
		Waveform_Date[i]=x;
		x=y;
	//	LCD_Draw_Solid_Point(x_min+i,y_max-Waveform_Date[i],BLUE);//画出新的点（大点）
    LCD_Fast_DrawPoint(x_min+i,y_max-Waveform_Date[i],BLUE);//画出新的点（小点）
		POINT_COLOR = BLUE;
		LCD_DrawLine(x_min+i,y_max-Waveform_Date[i],x_min+i-1,y_max-Waveform_Date[i-1]);//画出新的线
